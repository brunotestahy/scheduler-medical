...
import { fadeAnimationTrigger } from '../animations/fade-animation';
...
import { AppointmentStandard } from '../models/appointment-standard';
import { BaseRequestOptions } from '@angular/http';
import { AppointmentListService } from '../../services/appointment-list.service';
import { AppointmentList } from '../models/appointment-list';
import { RefreshService } from '../../services/refresh.service';
import { AppointmentCategory } from '../models/appointment-category';
...
export class SchedulerComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  yesterdayDate: Date = moment(this.activeDay).subtract(1, 'd').toDate();
  ...
  // Copy from yesterday checkboxes
  multiSelect = {
    categories: {},
    selectedCategory: '',
    allState: false
  ...
  loadGeneralListeners() {
    ...
    this.dateEventSubscription = this.scheduleService.handleDateChange$
      .subscribe((newDate: Date) => {
        ...
        this.yesterdayDate = moment(this.activeDay).subtract(1, 'd').toDate();
        ...
        if (this.customScheduleObject.appointmentList.length === 0 ||
          ...
          return this.appointmentListService.create(newAppointmentListObject)
            ...
            .flatMap((responseList) => {
              ...
              // Update the appointment list version to avoid 409 errors
              this.customScheduleObject.appointmentList.filter((appointmentList) => {
                return appointmentList.id === this.getCurrentAppointmentList(this.customScheduleObject.patientData.hisAdtId).id;
              })[0].version = responseList['version'];
  ...
  copyEventsFromYesterday() {
    // Catch selected categories
    const selectedCategories: string[] = [];
    for (const category in this.multiSelect.categories) {
      if (this.multiSelect.categories[category]) {
    ...
    const eventsYesterday: Event[] = JSON.parse(JSON.stringify(this.customScheduleObject.updatedEvents
      .filter((event) => {
        return event.start.slice(0, 10) === this.dateFormatter(dateYesterday).slice(0, 10) && !event.standard && !event.repeatEveryDay &&
          selectedCategories.includes(EnumEventCategory[event.category]);
      })));
    ...
    if (eventsYesterday.length === 0) {
      this.loadingActive = false;
      this.messageService.add({severity: 'warn', summary: 'Atenção!', detail: 'Não existem eventos no dia anterior'});
      return;
    }
    ...
    if (this.customScheduleObject.onlineMode) {
      ...
      if (newAppointmentObjects.length > 0) {
        ...
        let messageDetail = 'Você copiou um evento';
        if (newAppointmentObjects.length > 1) {
          messageDetail = `Você copiou ${newAppointmentObjects.length} eventos`;
        }
        this.messageService.add({severity: 'success', summary: 'Sucesso!', detail: messageDetail});
      } else {
        this.loadingActive = false;
        this.messageService.add({severity: 'warn', summary: 'Atenção!', detail: 'Os eventos do dia anterior já foram copiados.'});
      }
    } else {
      setTimeout(() => {
        this.buildScheduler();
  ...
  getEventCategoryFromAppointmentCategory(category: string): number {
    switch (category) {
      ...
      case EnumEventCategory[EnumEventCategory.procedimento]: {
        return EnumEventCategory.procedimento;
      }
      default: {
  ...
  buildAllEventsOnScheduler() {
    ...
    this.defaultScheduleObject = {
      ...
      newEventHeader: {
        active: true,
        title1: 'Olá, ',
        title2: '! Veja abaixo os eventos para hoje.',
      ...
      eventCategoryStyles: {
        ...
        procedimento: {
          eventBgColor: '#dee7e8',
          logoBgColor: '#c8e0e1',
          logoDescription: 'procedimento'
        },
        outros: {
          eventBgColor: '#ddddddf2',
          logoBgColor: '#dddddddb',
      ...
      for (let j = 0; j < events.length; j++) {
        ...
        if (equalCondition) {
          ...
          switch (events[j].category) {
            ...
            case EnumEventCategory.procedimento: {
              this.renderer.addClass(newImageLogo, 'logo-procedimento');
              this.renderer.setAttribute(newLogoWrapperElement, 'category', EnumEventCategory[EnumEventCategory.procedimento]);
              this.renderer.addClass(newLogoWrapperElement, 'fc-logo-event');
              this.renderer.setStyle(newLogoWrapperElement, 'background-color',
                this.customScheduleObject.eventCategoryStyles.procedimento.logoBgColor);
              this.renderer.setProperty(descriptionLogoElement, 'innerHTML',
                this.customScheduleObject.eventCategoryStyles.procedimento.logoDescription);
              this.renderer.insertBefore(eventsElements[i], newLogoWrapperElement, referenceElements[i]);
              break;
            }
  ...
  onEventDescriptionExistence() {
    ...
    const todayEvents = this.customScheduleObject.updatedEvents.filter((event) => {
      return event.start.includes(this.dateFormatter(this.activeDay).slice(0, 10)) && !event.allDay;
    });
    ...
    for (let i = 0; i < gridTimeElements.length; i++) {
      ...
      for (const eventObject of todayEvents) {
        // Condition is different between standard and ordinary events
        const equalCondition: boolean = eventObject.standard ?
          gridTimeElements[i].innerHTML.indexOf(eventObject.title) !== -1 :
          gridTimeElements[i].parentElement.id.includes(eventObject.id.toString());
        ...
        if (equalCondition &&
          (eventObject.description || eventObject.videoLink ||
            (gridTimeElements[i].parentElement.clientWidth < 250 && eventDuration <= minimumTime) ||
            (eventObject.title.length > 15 && eventDuration <= minimumTime))) {
    ...
    for (let i = 0; i < eventContents.length; i++) {
      ...
      this.customScheduleObject.updatedEvents
        ...
        .forEach((event) => {
          ...
          if (equalCondition && (event.description || event.videoLink)) {
      ...
      this.customScheduleObject.updatedEvents
        .forEach((event) => {
          ...
          if (equalCondition) {
            ...
            if (eventContents[i].parentElement.clientWidth < 200) {
              if (eventLogos[i]) {
                this.renderer.addClass(eventLogos[i].lastChild, 'ellipsis-event-title');
              }
            }
              ...
              if (eventDuration <= minimumTime) {
                ...
                for (let j = 0; j < eventLogos.length; j++) {
                  ...
                  const equalLogoCondition: boolean = event.standard ?
                    ...
                    eventLogos[j].parentElement.id.includes(event.id.toString());
  ...
  fillEventsBgColors() {
    ...
    for (const event of allEvents) {
      switch (event.category) {
        ...
        case EnumEventCategory.procedimento: {
          event.backgroundColor = this.customScheduleObject.eventCategoryStyles.procedimento.eventBgColor;
          break;
        }
        case EnumEventCategory.outros: {
  ...
  openDialog(type: string, action?: string, event?: any) {
    ...
    setTimeout(() => {
      ...
      // Copy from yesterday case
      if (type === 'copy-events') {
        this.fillCategoriesOnMultiSelect();
        return;
      }
  ...
  transferEventValuesToForm() {
    ...
    switch (this.customScheduleObject.currentEvent.category) {
      ...
      case EnumEventCategory.procedimento: {
        this.modalConfig.categoryEventStyle.selectedCategory = EnumEventCategory[EnumEventCategory.procedimento];
        break;
      }
      case EnumEventCategory.outros: {
  ...
  // Load the carousel using the Flickity library
  carouselLoader() {
  ...
  // Set the event id on each html element during the schedule's loading
  renderEvent(event, element, view) {
    element.attr('id', 'event-id-' + event.id);
  }

  // Fill the multiselect object with the categories from the server
  fillCategoriesOnMultiSelect() {
    this.customScheduleObject.appointmentCategory
      .forEach((category) => {
        this.multiSelect.categories[category.code] = false;
      });
  }

  // Action for check all events from yesterday or not
  onAllEventsChecked() {
    const keys = Object.keys(this.multiSelect.categories);

    keys.map((key) => {
      this.multiSelect.categories[key] = this.multiSelect.allState;
    });
  }

  // Action when some of the checkboxes is clicked in copy from yesterday
  onSomeCategoryChecked() {
    this.multiSelect.allState = Object.values(this.multiSelect.categories).every(value => !!value);
  }

  // Check if there is at least one checkbox checked
  atLeastOneCheckBoxValid(): boolean {
    return Object.values(this.multiSelect.categories).some(value => !!value);
  }
}
